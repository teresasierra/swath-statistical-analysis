...
# calculate Shapiro-Wilk Normality Tests for each set
cal2_df_row_cn = nrow(cal1_filtered_df)
row_group_v = c()
new_group_set_col_v = c()
for(set_name in names(calc1_col_nam_sets)){
  ...
  for( row in 1:cal2_df_row_cn) { # cal2_df_row_cn
    ...
    row_group_v = cal1_filtered_df[row, new_group_set_col_v]
    # print(row_group_v)
    set_shapiro = shapiro.test(unlist(row_group_v))
   # print(paste( 'Shapiro', set_shapiro))
    cat('\n')
    cal2_output_df[row, paste('Shapiro.', set_name, sep='')] = set_shapiro$p.value

  }
}


# calculate Bartlett to test for equal variance between all sets
for( row in 1:cal2_df_row_cn) {
  list_barlett_sets = list()
  for(set_name in names(calc1_col_nam_sets)){
    row_set_values_name_v = cal1_filtered_df[row, calc1_col_nam_sets[[set_name]]]
    list_barlett_sets[[length(list_barlett_sets) + 1]] =unlist(row_set_values_name_v, recursive = TRUE, use.names = FALSE) 
  }
  
  # print(list_barlett_sets)
  set_bartlett = bartlett.test(list_barlett_sets)
  # print(paste( 'Bartlett', set_bartlett))
  cat('\n')
  cal2_output_df[row, paste('Bartlett.', set_name, sep='')] = set_bartlett$p.value
  
  # print(set_bartlett$p.value)
}
